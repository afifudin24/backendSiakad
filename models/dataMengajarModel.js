...
const DataMengajar = {
  getDataMengajar: callback => {
    db.query(
      ' SELECT  datamengajar.id AS datamengajar_id, datamengajar.jamMengajar, datamengajar.pertemuanPerMinggu,  gurus.id AS guru_id,  gurus.nama AS guru_nama,  kelas.id AS kelas_id,  kelas.title AS kelas_nama, kelas.tingkat AS kelas_tingkat,  mapel.id AS mapel_id,  mapel.namaMapel AS mapel_nama FROM datamengajar JOIN kelas ON datamengajar.kelas_id = kelas.id JOIN mapel ON datamengajar.mapel_id = mapel.id JOIN gurus ON datamengajar.guru_id = gurus.id ',
      callback
    )
  },
