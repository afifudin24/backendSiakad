...
class Schedule {
  ...
  isSlotAvailable (slot, classItem) {
    ...
    // Get the configuration for the day
    const isFriday = slot.day === 5
    const config = isFriday ? TIME_CONFIG.FRIDAY : TIME_CONFIG.REGULAR_DAY

    // Calculate the end time for this slot
    const slotStartTime = new Date(slot.startTime)
    const slotEndTime = new Date(
      slotStartTime.getTime() + classItem.minutesPerMeeting * 60000
    )
    const configEndTime = getTimeFromString(config.END_TIME)

    // Check if the class would extend beyond the day's end time
    if (slotEndTime > configEndTime) {
      return false
    }
